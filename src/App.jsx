...
const GAME_DEFS = [
  { id: "whoami", title: "מי אני?", desc: "דמות מפורסמת על המצח — נחשו לפני שנגמר הזמן", icon: Brain, color: COLORS.purple },
...
const GENERIC_PERSONAS = [
  ...
  // --- ציירים, פסלים ואמני עבר ---
  "לאונרדו דה וינצ'י", "מיכלאנג'לו", "וינסנט ואן גוך", "פבלו פיקאסו",
  "סלבדור דאלי", "קלוד מונה", "פרידה קאלו", "רמברנדט", "אנדי וורהול",

  // --- דמויות מסרטים וסדרות (בינלאומי וישראלי) ---
  "הארי פוטר", "וולדמורט", "הרמיוני גריינג'ר", "דמבלדור",
  "דארת' ויידר", "לוק סקייווקר", "יודה",
  "ג'ק ספארו", "ג'יימס בונד", "אינדיאנה ג'ונס", "הג'וקר", "באטמן",
  "ספיידרמן", "סופרמן", "וולברין", "טוני סטארק (איירון מן)",
  "פורסט גאמפ", "ג'ק דוסון (טיטניק)", "טרמינטור", "ניאו (מטריקס)",
  "וולטר וייט", "טוני סופרנו", "רייצ'ל גרין (חברים)", "רוס גלר (חברים)",
  "ג'ואי טריביאני (חברים)", "מוניקה גלר (חברים)", "צ'נדלר בינג (חברים)",
  "שרלוק הולמס", "ג'ון סנואו", "דאינריז טארגאריין", "ונסדיי אדמס",
  "מירי פסקל", "שאולי", "ראובן (הפרלמנט)", "אבי הטחול", "קוגלביץ'",
  ...
  "סקובי דו", "גארפילד", "פו הדב", "גרינץ'",
  "דרדסבא", "גרגמל", "פופאי", "פיקאצ'ו",

  // --- מוזיקה ישראלית: קלאסיקות ועכשווי ---
  ...
  // --- מנהיגים ופוליטיקאים מהעבר וההווה (ישראל) ---
  "דוד בן גוריון", "גולדה מאיר", "מנחם בגין", "יצחק רבין", "שמעון פרס",
  "משה דיין", "זאב ז'בוטינסקי", "בנימין זאב הרצל", "בנימין נתניהו",
  "אריאל שרון", "לוי אשכול", "יגאל אלון", "חיים ויצמן",

  // --- מנהיגים ופוליטיקאים בעולם ---
  "וינסטון צ'רצ'יל", "אברהם לינקולן", "ג'ון פ. קנדי", "פרנקלין רוזוולט",
  "מרגרט תאצ'ר", "נלסון מנדלה", "נפוליאון בונפרטה", "יוליוס קיסר",
  "אלכסנדר הגדול", "ברק אובמה", "דונלד טראמפ", "המלכה אליזבת השנייה",
  "קלאופטרה", "מהטמה גנדי",

  // --- אנשי מדע, רוח והגות ---
  "אלברט איינשטיין", "אייזק ניוטון", "צ'ארלס דרווין", "גלילאו גליליי",
  "מארי קירי", "זיגמונד פרויד", "וויליאם שייקספיר", "חנה סנש",

  // --- בידור, קולנוע וטלוויזיה (ישראל) ---
  "אפרים קישון", "שייקה אופיר", "יוסי בנאי", "ספי ריבלין", "זאב רווח",
  "יהודה ברקן", "טוביה צפיר", "ארז טל", "אסי עזר", "רותם סלע",
  "גיא פינס", "אדיר מילר", "אלי פיניש", "מריאנו אידלמן", "טל פרידמן",
  "איל קיציס", "אופירה אסייג", "איל ברקוביץ'", "יובל המבולבל",

  // --- כוכבי קולנוע ומוזיקה בינלאומיים ---
  "צ'ארלי צ'פלין", "מרילין מונרו", "אלביס פרסלי", "מייקל ג'קסון",
  "פרדי מרקיורי", "ג'ון לנון", "פול מקרטני", "בוב דילן", "פרנק סינטרה",
  "טום קרוז", "לאונרדו דיקפריו", "בראד פיט", "מרלון ברנדו", "רוברט דה נירו",
  "אודרי הפבורן", "מריל סטריפ", "אנג'לינה ג'ולי", "טיילור סוויפט", "ביונסה",

  // --- ספורטאי על מכל הזמנים ---
...
function WhoAmIGame({ players, category, spice, onScoreUpdate, onBack }) {
  ...
  }, [phase, timeLeft, correct, current.id, onScoreUpdate, skips]);
...
function ShrinkGame({ players, onScoreUpdate, onBack }) {
  ...
  }, [phase, psych, diagnosedCount, onScoreUpdate, totalScore]);
